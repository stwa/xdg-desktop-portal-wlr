	...
	if (xdpw_pwr_is_driving(cast)) {
		if (cast->frame_state == XDPW_FRAME_STATE_SUCCESS) {
			...
			if (delay_ns > 0) {
				...
				cast->frame_state = XDPW_FRAME_STATE_FINISHED;
				logprint(TRACE, "wlroots: finish screencopy finished with timer");
				return;
			}
		}
		logprint(TRACE, "wlroots: triggering next round via pipewire");
		xdpw_pwr_trigger_process(cast);
	}
	logprint(TRACE, "wlroots: finish screencopy finished");
	cast->frame_state = XDPW_FRAME_STATE_FINISHED;
}
...
void xdpw_wlr_frame_start(struct xdpw_screencast_instance *cast) {
	...
	if (cast->frame_state != XDPW_FRAME_STATE_NONE && cast->frame_state != XDPW_FRAME_STATE_FINISHED) {
		logprint(WARN, "wlroots: frame start was called out of time");
		cast->err = true;
	}
	...
	cast->frame_state = XDPW_FRAME_STATE_CREATED;
